...
let VIDEO_CACHE_SETTINGS = { steamApiKey: '', useSteamApi: false, customPlayers: [], defaultPlayer: 'builtin' }; // Default settings
// 确保 Steam 配置目录存在
...
async function scrapeIds(params) {
  ...
  const tags = [];
    const typesArr = Array.isArray(params.types) ? params.types : [];
    const ratingsArr = Array.isArray(params.ratings) ? params.ratings : [];
    if (typesArr.length === 1) tags.push(String(typesArr[0]).trim());
    if (ratingsArr.length === 1) tags.push(String(ratingsArr[0]).trim());
...
function mapLocalQueryTypeToSteamApi(localType) {
  ...
  if (n === 7) return 0;  // 最高评分
  return 3;
}

function buildSteamApiQueryFields(params) {
  const requiredTags = [];
  const excludedTags = [];

  const ALL_TYPES = ['Scene', 'Video', 'Web', 'Application'];
  const ALL_RATINGS = ['Everyone', 'Questionable', 'Mature'];

  const typesLocal = Array.isArray(params.types) ? params.types : [];
  const ratingsLocal = Array.isArray(params.ratings) ? params.ratings : [];

  ALL_TYPES.forEach(t => { if (!typesLocal.includes(t)) excludedTags.push(t); });
  ALL_RATINGS.forEach(r => { if (!ratingsLocal.includes(r)) excludedTags.push(r); });

  const genreOr = [];
  if (Array.isArray(params.genre_or)) params.genre_or.forEach(g => g && genreOr.push(String(g).trim()));
  for (const [k, v] of Object.entries(params)) {
    if (/^genre_or\[\d+\]$/.test(k) && v) genreOr.push(String(v).trim());
  }
  
  genreOr.forEach(g => { if (g) requiredTags.push(g); });
...
async function queryWorkshopBySteamApi(apiKey, params) {
  if (params.creator) {
    const qsUser = [
  ...
  const q = buildSteamApiQueryFields(params);
  ...
  q.excludedTags.forEach((tag, i) => search.set(`excludedtags[${i}]`, tag));
  ...
  return { ids: oneIds, totalCount: parseInt(resp.total || 0) || 0, detailMap };
}
...
async function handleQuery(req, res) {
  ...
  const typesLocal = Array.isArray(params.types) ? params.types.map(t => String(t).toLowerCase()) : [];
  const ratingsLocal = Array.isArray(params.ratings) ? params.ratings.map(t => String(t).toLowerCase()) : [];

  const filterItemLocally = (d) => {
    if (!d || d.result !== 1) return false;
    const tagSet = new Set((d.tags || []).map(t => String(t.tag || t).toLowerCase()));
    
    if (genreOr.length > 0 && !genreOr.some(g => tagSet.has(g))) return false;
    if (typesLocal.length > 0 && !typesLocal.some(t => tagSet.has(t))) return false;
    if (ratingsLocal.length > 0 && !ratingsLocal.some(r => tagSet.has(r))) return false;
    return true;
  };
  ...
  try {
    ...
    while (matched.length < numperpage && scanned < 6 && cursorPage <= 999) {
      ...
      if (useSteamApi) {
        sourceData = await queryWorkshopBySteamApi(steamApiKey, pageParams);
        if (!sourceData.ids || !sourceData.ids.length) {
          console.warn(`[Query] SteamAPI page ${cursorPage} empty, fallback to scrapeIds`);
          sourceData = await scrapeIds(pageParams);
        } else {
          sourceData.hints = {};
        }
      } else {
      ...
      if (!totalCount && sourceData.totalCount) totalCount = sourceData.totalCount;
      if (!sourceData.ids || !sourceData.ids.length) break;

      let details = [];
      if (useSteamApi && sourceData.detailMap && Object.keys(sourceData.detailMap).length > 0) {
        details = sourceData.ids.map(id => sourceData.detailMap[id]).filter(Boolean);
      } else {
        try { details = await getFileDetailsSafe(sourceData.ids); }
      ...
      for (const id of sourceData.ids) {
        if (seen.has(id)) continue;
        seen.add(id);
        const d = detailMap[id];
        // 如果当前壁纸符合本地筛选条件，就加入列表；如果不符合，循环会继续跑去借用下一个壁纸
        if (filterItemLocally(d)) {
          matched.push(mapItem(id, d, (sourceData.hints && sourceData.hints[id]) || {}));
    ...
    const total = totalCount > 0 ? totalCount : (matched.length >= numperpage ? 50000 : matched.length);
    console.log(`[Query] Returning ${matched.length} items, total=${total}, scanned=${scanned}`);
...
async function handleDetails(res, id) {
  ...
  // 提取真实的创作者 ID
  const apiCreatorId = A && A.creator ? String(A.creator) : '';
  ...
  const commentsTask = (async () => {
    try {
      // 评论接口传入作者 ID 
      if (!apiCreatorId) return []; 
      const cUrl = `https://steamcommunity.com/comment/PublishedFile_Public/render/${apiCreatorId}/${id}/`;
...
function parseComments(html) {
  ...
  const blocks = html.split(/class="[^"]*commentthread_comment_content[^"]*"/).slice(1);
  
  for (const block of blocks) {
    const authorMatch = block.match(/class="[^"]*commentthread_author_link[^"]*"[^>]*>([\s\S]*?)<\/a>/);
    const dateMatch = block.match(/class="[^"]*commentthread_comment_timestamp[^"]*"[^>]*>([\s\S]*?)<\/span>/);
    const textMatch = block.match(/class="[^"]*commentthread_comment_text[^"]*"[^>]*>([\s\S]*?)<\/div>/);
    
    if (textMatch) {
      const author = (authorMatch ? authorMatch[1] : '').replace(/<[^>]+>/g, '').trim() || 'Steam User';
...
async function zipDir(dirPath, zipPath) {
  ...
  if (process.platform === 'win32') {
    const cmd = `Compress-Archive -Path '${psQuote(path.join(dirPath, '*'))}' -DestinationPath '${psQuote(path.resolve(zipPath))}' -Force`;
    await runProcess('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', cmd], 180000);
  } else {
    const zipArgs = ['-r', path.resolve(zipPath), '.'];
...
async function downloadViaSteamCmd(publishedFileId, appId, title, options) {
  ...
  try {
    ...
    // 非视频类壁纸直接在后台打包成 ZIP 压缩包
    const folderName = `${safeName(title || `Wallpaper ${publishedFileId}`)}-${publishedFileId}`;
    const zipName = `${folderName}.zip`;
    const zipPath = path.join(os.tmpdir(), zipName);
    console.log(`[SteamCMD] Packaging folder to zip: ${itemDir} -> ${zipPath}`);

    await zipDir(itemDir, zipPath);

    return { kind: 'file', filePath: zipPath, fileName: zipName, tempRoot, useSharedDir, itemDir: itemDir };
...
async function handleDownload(res, id, title) {
  ...
  // 提取真实分类并存入队列缓存，供前端标签显示
  let type = 'Scene';
  const ts = (d.tags || []).map(t => String(t.tag || t).toLowerCase());
  if (ts.includes('video')) type = 'Video';
  else if (ts.includes('application')) type = 'App';
  else if (ts.includes('web')) type = 'Web';

  const itemTitle = safeName(title || d.title || `Wallpaper ${wantId}`) || `Wallpaper ${wantId}`;

  // 将任务推入队列，并立即向前端返回
  const task = {
    id: wantId, appId: appId, title: itemTitle, isVideo: isVideo, type: type, sourceUrl: sourceUrl,
  ...
  try {
    ...
    console.warn('[Queue] 提前创建下载占位文件夹失败:', e.message);
  }
...
async function recoverQueueFromSteamTemp() {
  ...
  const isAlreadyDownloaded = (id) => {
    try {
      ...
      return files.some(f => f.includes(String(id)));
    } catch (e) { return false; }
  };

  // 3. 过滤出真正需要恢复的 ID
  const idsToRecover = Array.from(foundIds).filter(id => 
    !TASK_QUEUE.some(t => String(t.id) === String(id)) && !isAlreadyDownloaded(id)
  );
  ...
  try {
    ...
    idsToRecover.forEach(id => {
      ...
      if (d) {
        ...
        else if (tags.includes('web')) type = 'Web';
  ...
  try {
      ...
      try {
        if (fs.existsSync(DOWNLOAD_DIR)) {
          ...
          for (const f of files) {
            ...
            if (idMatch) downloadedMap[idMatch[0]] = f;
          }
        }
      } catch (e) {}
      return jsonRes(res, 200, { tasks: TASK_QUEUE, rxSpeed: Math.max(0, currentRxSpeed), downloadedMap });
    }

    // 补充被遗漏的本地库直接下载 API
    if (pn==='/api/library/download' && req.method==='GET') {
      try {
        const q = new URL(req.url, 'http://x').searchParams;
        const targetName = path.basename(q.get('name') || '');
        if (!targetName) return jsonRes(res, 400, { error: '参数无效' });
        
        const targetPath = path.join(DOWNLOAD_DIR, targetName);
        if (!fs.existsSync(targetPath)) return jsonRes(res, 404, { error: '文件不存在' });

        const stat = fs.statSync(targetPath);
        res.setHeader('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(targetName)}`);
    ...
    if (pn==='/api/video/cache/settings' && req.method==='GET') {
      jsonRes(res, 200, {
        ...
        customPlayers: VIDEO_CACHE_SETTINGS.customPlayers || [],
        defaultPlayer: VIDEO_CACHE_SETTINGS.defaultPlayer || 'builtin'
      }); return;
    }
    if (pn==='/api/video/cache/settings' && req.method==='POST') {
      try {
        ...
        if (Object.prototype.hasOwnProperty.call(data, 'customPlayers')) {
          VIDEO_CACHE_SETTINGS.customPlayers = data.customPlayers;
        }
        if (Object.prototype.hasOwnProperty.call(data, 'defaultPlayer')) {
          VIDEO_CACHE_SETTINGS.defaultPlayer = data.defaultPlayer;
        }
        saveCacheSettings();
        jsonRes(res, 200, {
          success: true,
    ...
    // 获取图库列表
    if (pn==='/api/library' && req.method==='GET') {
      try {
        if (!fs.existsSync(DOWNLOAD_DIR)) return jsonRes(res, 200, { items: [] });
        const files = fs.readdirSync(DOWNLOAD_DIR);
        const items = [];
        for (const f of files) {
          if(f.startsWith('.')) continue; 
          ...
          items.push({
            name: f, size: stat.size, isDir: stat.isDirectory(), mtime: stat.mtime.getTime(), id: id
          });
        }
        items.sort((a, b) => b.mtime - a.mtime); // 按时间倒序排列
    ...
    // 支持随意拖拽进度
    if (pn.startsWith('/api/library/play') && req.method==='GET') {
      try {
        const q = new URL(req.url, 'http://x').searchParams;
        const targetName = path.basename(q.get('name') || '');
        if (!targetName) return jsonRes(res, 400, { error: '参数无效' });
        ...
        if (range) {
          ...
          res.writeHead(206, {
            ...
            'Content-Type': mimeType(targetPath) || 'video/mp4',
          });
          file.pipe(res);
        } else {
          // 浏览器首次请求（无进度标记）
    ...
    // 批量异步获取 Steam 详细数据接口
    if (pn==='/api/library/details' && req.method==='POST') {
      try {
        const { ids } = JSON.parse(await readBody(req));
        if (!ids || !ids.length) return jsonRes(res, 200, { details: {} });
        const details = await getFileDetailsSafe(ids);
        const detailMap = {};
        details.forEach(d => { if (d && d.publishedfileid) detailMap[d.publishedfileid] = d; });
        return jsonRes(res, 200, { details: detailMap });
      } catch (e) {
        return jsonRes(res, 500, { error: e.message });
      }
    }
    ...
    // 单个安全删除壁纸
    if (pn==='/api/library/delete' && req.method==='POST') {
      try {
        const data = JSON.parse(await readBody(req));
        const targetName = path.basename(data.name || ''); 
        if (!targetName) return jsonRes(res, 400, { error: '参数无效' });
        ...
        const idMatch = targetName.match(/\d{8,}/);
        const targetId = idMatch ? idMatch[0] : null;
        if (targetId) {
          const qIdx = TASK_QUEUE.findIndex(t => String(t.id) === targetId);
          if (qIdx !== -1) {
            TASK_QUEUE.splice(qIdx, 1);
          }
        }
    ...
    // 批量删除本地壁纸接口
    if (pn==='/api/library/delete_batch' && req.method==='POST') {
      try {
        const data = JSON.parse(await readBody(req));
        ...
        for (const name of names) {
          ...
          if (targetId) {
            const qIdx = TASK_QUEUE.findIndex(t => String(t.id) === targetId);
            if (qIdx !== -1) TASK_QUEUE.splice(qIdx, 1);
          }
        }

        return jsonRes(res, 200, { success: true, deletedCount });
      } catch (e) {
        return jsonRes(res, 500, { error: e.message });
      }
    }
    ...
    // 获取与更新全局壁纸元数据映射表的 API
    if (pn==='/api/library/meta' && req.method==='GET') {
      return jsonRes(res, 200, libraryMetaCache);
    }
    if (pn==='/api/library/meta' && req.method==='POST') {
      try {
        const data = JSON.parse(await readBody(req));
        Object.assign(libraryMetaCache, data);
        saveLibraryMeta();
        return jsonRes(res, 200, { success: true });
      } catch (e) {
    ...
    if (pn==='/api/library/delete' && req.method==='POST') {
      try {
        ...
        // 删除文件后，提取壁纸ID，清理记录与残留队列
        const idMatch = targetName.match(/\d{8,}/);
        const targetId = idMatch ? idMatch[0] : null;
        if (targetId) {
          // 1. 删除存折文件中的记录
          if (libraryMetaCache[targetId]) {
            delete libraryMetaCache[targetId];
            saveLibraryMeta();
          }
          // 2. 如果下载队列里还有这个任务（已完成状态的残留），直接剔除
          const qIdx = TASK_QUEUE.findIndex(t => String(t.id) === targetId);
          if (qIdx !== -1) {
            TASK_QUEUE.splice(qIdx, 1);
          }
    ...
    // 动态解析外部播放器的命令行脚本模板 (升级版：支持多重嵌套自内向外解析)
    if (req.method === 'POST' && pn === '/api/library/resolve_url') {
      let body = '';
      req.on('data', c => body += c);
      req.on('end', async () => {
        try {
          const { template, name, fullPath } = JSON.parse(body);
          
          // 1. 先替换基础变量 <url> 和 <name>
          let finalUrl = template
            .replace(/<url>/g, fullPath)
            .replace(/<name>/g, name);

          const execCmd = (cmdStr) => new Promise((resolve) => {
            require('child_process').exec(cmdStr, { timeout: 10000 }, (err, stdout) => {
              if (err) console.warn('[Nested Cmd Player Resolve Error]', err.message);
              resolve(err ? '' : stdout.trim());
            });
          });

          // 2. 使用不包含嵌套的最小闭合正则：匹配最内层、纯净的 <cmd>...</cmd>
          // [^>]* 是为了防止内层包含不规范的标签字符，仅匹配不含子 <cmd> 的最内层块
          const innerCmdRegex = /<cmd>((?:(?!<cmd>)[\s\S])+?)<\/cmd>/;

          let safetyCounter = 0; // 防止陷入死循环
          while (innerCmdRegex.test(finalUrl) && safetyCounter < 10) {
            safetyCounter++;
            const match = finalUrl.match(innerCmdRegex);
            if (!match) break;

            const fullTag = match[0]; // 例如: <cmd>解析token的代码</cmd>
            const innerCmdText = match[1]; // 例如: 解析token的代码

            // 执行最内层脚本
            const resultText = await execCmd(innerCmdText);

            // 核心细节：如果是最外层的顶级解析，结果需要 URL 编码（encodeURIComponent）以拼接网址；
            // 如果它还在别的 <cmd> 内部（说明是给外层脚本提供参数的），必须保持原样字符串，不能编码，否则外层 curl 执行会报错！
            const isOutermost = !finalUrl.replace(fullTag, '').includes('</cmd>');
            const replacement = isOutermost ? encodeURIComponent(resultText) : resultText;

            // 将其替换回主字符串，进入下一轮循环（外层 <cmd> 此时就变成了不含嵌套的最内层）
            finalUrl = finalUrl.replace(fullTag, replacement);
          }

          return jsonRes(res, 200, { url: finalUrl });
        } catch (e) {
          return jsonRes(res, 500, { error: e.message });
        }
      });
...
server.listen(PORT,'0.0.0.0',()=>{
  ...
  // 服务器启动时扫描 Steam 临时目录并恢复队列
  recoverQueueFromSteamTemp();
