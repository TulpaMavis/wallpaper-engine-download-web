...
async function fetchQueue() {
  try {
    ...
    container.innerHTML = list.map(t => {
      ...
      return `
      <div class="q-item">
        <img class="q-thumb" src="${t.thumb || PLACEHOLDER}" alt="">
        ...
        <div class="q-content">
          ...
          <div class="q-actions">
            ...
            <div class="q-updown">
              <button class="q-btn q-btn-half top" title="上移" onclick="qAction('up', ${t.id})">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="18 15 12 9 6 15"/></svg>
              </button>
              <button class="q-btn q-btn-half btm" title="下移" onclick="qAction('down', ${t.id})">
            ...
            ${(t.status === 'completed' || t.status === 'error') 
              ? `<button class="q-btn" onclick="qAction('cancel', ${t.id})">清理</button>`
              : `<button class="q-btn danger" onclick="promptCancelTask(${t.id})">取消</button>`
            }

            ${(t.status === 'downloading' || t.status === 'pending') ? `<button class="q-btn warn" onclick="qAction('pause', ${t.id})">暂停</button>` : ''}
            ${(t.status === 'paused' || t.status === 'error') ? `<button class="q-btn success" onclick="qAction('resume', ${t.id})">继续</button>` : ''}
          </div>
        </div>
      </div>`;
    }).join('');
