...
function openModal(id, fallbackTitle='', fallbackThumb='', fallbackType=''){
  ...
  // 注入弹窗顶部的类型贴纸
  let mType = isFromCache ? fallbackType : getType(item);
  ...
  fetch(`/api/steam/details?id=${id}`)
    ...
    .then(d=>{
      if (d.preview_url && document.getElementById('mImg').src.includes('svg')) {
        document.getElementById('mImg').src = d.preview_url;
      }
      if (d.tags && d.tags.length) {
        const realType = getType(d);
        const realTypeText = realType === 'Video' ? '视频' : (realType === 'Web' ? '网站' : (realType === 'App' ? '应用' : '场景'));
        const badgeEl = document.getElementById('mTypeBadge');
        if (badgeEl) {
          badgeEl.className = `type-badge ${realType.toLowerCase()}`;
          badgeEl.textContent = realTypeText;
        }
        document.getElementById('mTags').innerHTML = d.tags.map(t=>`<span class="tag-chip">${esc(translateTag(t.tag||t))}</span>`).join('');
      } else {
        const tags=(item.tags||[]).map(t=>t.tag||t).filter(Boolean);
  ...
  try {
    ...
    container.innerHTML = list.map((t, idx) => {
      ...
      if (cached && cached.type) {
        ...
        typeName = cached.type === 'Video' ? '视频' : (cached.type === 'Web' ? '网站' : (cached.type === 'App' ? '应用' : '场景'));
        badgeDisplay = ''; // 查到了真实类型，解除隐藏
      } else if (cached || !id) {
        // 如果缓存存在但没查到类型（证明查过但搜不到），或者是纯本地无ID文件，解除隐藏并显示兜底的基础文件类型
        badgeDisplay = '';
      }
      ...
      // 将打开详情页的事件转移到壁纸名称上，并加入鼠标经过变色的反馈提示，同时传参真实Type
      return `
      ...
      .then(d => {
        ...
        list.forEach((t, idx) => {
          ...
          if (!detailMap[id]) {
            ...
            libLocalCache[id] = { subs: 0, favs: 0, thumb: '', type: '' }; // 空数据也进缓存，防二次请求
            cacheUpdated = true;
            
            // 异步无数据，强制显示兜底角标
            const badgeEl = document.querySelector(`#lib-item-${idx} .type-badge`);
            if (badgeEl) {
              const isVid = t.name.match(/\.(mp4|webm|mov|mkv|ogg)$/i);
              badgeEl.className = `type-badge ${isVid ? 'video' : 'scene'}`;
              badgeEl.textContent = isVid ? '视频' : (t.isDir ? '文件夹' : '压缩包');
              badgeEl.style.display = '';
            }
            return;
          }
          ...
          let type = getType(itemD);
          
          // 新增核心修复：如果从 Steam 拿到了空壳数据连封面都没有，证明这是个残缺失效的壁纸详情，强制清空类型以走兜底
          if (!thumb) {
            type = '';
          }

          // 记录进缓存
          libLocalCache[id] = { subs, favs, thumb, type };
          cacheUpdated = true;
          ...
          if (badgeEl) {
            if (type) {
              badgeEl.className = `type-badge ${type.toLowerCase()}`;
              badgeEl.textContent = type === 'Video' ? '视频' : (type === 'Web' ? '网站' : (type === 'App' ? '应用' : '场景'));
            } else {
              const isVid = t.name.match(/\.(mp4|webm|mov|mkv|ogg)$/i);
              badgeEl.className = `type-badge ${isVid ? 'video' : 'scene'}`;
              badgeEl.textContent = isVid ? '视频' : (t.isDir ? '文件夹' : '压缩包');
            }
            badgeEl.style.display = ''; // 网络请求完毕，无论有没有查到，都解除隐藏显示对应角标
          }
        });
