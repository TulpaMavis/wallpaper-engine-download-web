  ...
  try {
    ...
    if (!hasGenreOr) {
      let sourceData = useSteamApi
        ? Object.assign({ hints: {} }, await queryWorkshopBySteamApi(steamApiKey, params))
      ...
      const items = [];
            for (const id of ids) {
              ...
              if (filterItemLocally(d)) {
                items.push(mapItem(id, d, (hints && hints[id]) || {}));
                if (items.length >= numperpage) break;
              }
            }
            const total = totalCount > 0 ? totalCount : (ids.length >= params.numperpage ? 50000 : ids.length);
    ...
    if (useSteamApi) {
      let apiData = await queryWorkshopBySteamApi(steamApiKey, params);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // 支持随意拖拽进度
    if (pn==='/api/library/play' && req.method==='GET') {
      try {
        const q = new URL(req.url, 'http://x').searchParams;
        const targetName = path.basename(q.get('name') || '');
        if (!targetName) return jsonRes(res, 400, { error: '参数无效' });
