...
function updateAllWallButtons() {
  document.querySelectorAll('.sub-btn, #mSubBtn').forEach(btn => {
    ...
    if (dlFileName) {
       ...
       btn.innerHTML = `<i style="font-style:normal; margin-right:4px;">⏳</i> ${task.status === 'downloading' ? '下载中' : '已排队'}`;
  ...
  try {
    ...
    container.innerHTML = list.map(t => {
      ...
      const thumbHtml = previewUrl 
        ? `<img src="${previewUrl}" class="q-thumb" onclick="openModal('${t.id}', '${safeTitle}', '${previewUrl}')" style="cursor:pointer;" title="查看详情" onerror="this.src='${PLACEHOLDER}'">` 
        : `<div class="q-thumb" onclick="openModal('${t.id}', '${safeTitle}')" style="cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:24px;">📄</div>`;
      ...
      <div class="q-item">
        ...
        <div class="q-content" style="flex: 1; min-width: 0;">
          <div class="q-head">
            <span class="q-title" title="${esc(t.title || t.id)}">${esc(t.title || t.id)}</span>
...
// 视频播放功能
function playLibraryItem(name) {
  if (name.endsWith('.zip')) {
    return toast('文件夹压缩包无法直接播放，请下载后解压查看', 'warn');
  }
  if (!name.match(/\.(mp4|webm|mov|mkv|ogg)$/i)) {
    return toast('当前文件不是支持的网页视频格式', 'warn');
  }
  ...
  if (ov && player) {
    // 指向支持分段流式传输的后端接口
    player.src = `/api/library/play?name=${encodeURIComponent(name)}`;
    ov.classList.add('open');
    document.body.style.overflow = 'hidden';
    player.play().catch(()=>{}); // 自动尝试播放
  }
}
...
function closeVideoModal() {
  ...
  if (ov && player) {
    ...
    document.body.style.overflow = '';
    player.pause();
    player.src = ''; // 🌟 清空 src 瞬间释放浏览器和后端的资源占用
  }
}
...
async function fetchLibrary() {
  ...
  try {
    ...
    container.innerHTML = list.map((t, idx) => {
      ...
      const thumbHtml = `<img id="lib-thumb-${idx}" src="${thumbSrc}" class="q-thumb" ${id ? `onclick="if(!libBatchMode) openModal('${id}', '${safeTitle}', '${cached && cached.thumb ? cached.thumb : ''}')"` : ''} style="${libBatchMode?'cursor:default;':'cursor:pointer;'}" title="查看详情" onerror="this.src='${PLACEHOLDER}'">`;
      ...
      <div class="q-item" id="lib-item-${idx}" style="position: relative; padding: 12px; display: flex; align-items: center; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; min-height: 94px;">
        ${chkHtml}
        ${thumbHtml}
        <div style="display: flex; flex-direction: column; gap: 2px; overflow: hidden; flex: 1; margin-left: 14px; justify-content: center;">
          <span style="font-weight: 600; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-right: 68px;" title="${esc(displayTitle)}">${esc(displayTitle)}</span>
        ...
        <button class="btn-p lib-play-btn" style="display: ${libBatchMode ? 'none' : 'flex'}; position: absolute; bottom: 12px; right: 12px; width: 62px; height: 28px; align-items: center; justify-content: center; padding: 0; font-weight: normal; font-size: 13px; border-radius: 6px; background: var(--success); border: 1px solid var(--success); color: #fff; cursor: pointer; white-space: nowrap;" onclick="playLibraryItem('${safeName}')">
      ...
      .then(d => {
        ...
        list.forEach((t, idx) => {
          ...
          if (thumbEl) {
            if (thumb) {
              thumbEl.src = thumb;
              const displayTitle = t.name.replace(/\.[^/.]+$/, "").replace(new RegExp(`[-_]?${id}.*$`), "") || id;
              const safeTitle = esc(displayTitle).replace(/'/g, "\\'");
              thumbEl.setAttribute('onclick', `if(!libBatchMode) openModal('${id}', '${safeTitle}', '${thumb}')`);
            } else {
