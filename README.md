...
- **自动打包规则**：
  - **场景/程序/网页类壁纸**：下载后整个文件夹会被移入downloads文件夹中
...
| `STEAM_API_KEY` | 通过API[获取连接](https://steamcommunity.com/dev/apikey)访问wallpaper，列表更全 | - |
...
## 二创声明
   ...
   - 加了个登入账号固化（重启不失效），下载队列（原版多个订阅会一起下载，容易出问题，有队列后会依次下载。会显示下载进度和容器的网速），查看作者的其他作品（方便找其他壁纸），优化了底部选页栏（可跳转）
   - 非视频类不打包成zip主要方便提取，比如用[RePKG](https://github.com/notscuffed/repkg)工具，通过命令可以批量提取文件夹(因为是exe工具，不方便集成进docker)。提取后可以找到原图，很多用静态壁纸的地方就可以用。
   - 随手用AI改的基本不会更新。
